...
from common.diagnostics import compute_yearly_vocab_size  # noqa: E402
...
def main() -> None:
    ...
    output_path = resolve_repo_path(args.output) if args.output else build_experiment_paths(experiment_id, output_root=args.output_root).diagnostics_dir / "yearly_vocab_size.csv"
    assert output_path is not None
    output_path.parent.mkdir(parents=True, exist_ok=True)

    frame = compute_yearly_vocab_size(stage1_dir)
    frame.to_csv(output_path, index=False)
    print(f"yearly vocab size saved to: {output_path}")
