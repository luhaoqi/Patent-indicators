...
from common.diagnostics import compute_yearly_top_vocab  # noqa: E402
...
def main() -> None:
    ...
    output_path = resolve_repo_path(args.output) if args.output else build_experiment_paths(experiment_id, output_root=args.output_root).diagnostics_dir / f"yearly_top_vocab_top{args.k}.csv"
    assert output_path is not None
    output_path.parent.mkdir(parents=True, exist_ok=True)

    frame = compute_yearly_top_vocab(stage1_dir, topk=args.k)
    frame.to_csv(output_path, index=False)
    print(f"yearly top vocab saved to: {output_path}")
